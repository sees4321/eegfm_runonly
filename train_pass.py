...
def build_parser(add_help: bool = True) -> argparse.ArgumentParser:
    ...
    p.add_argument("--dsize", type=int, default=1.0)
    return p
...
def relational_kl_loss(z: torch.Tensor, s: torch.Tensor, tau_z: float = 0.1, tau_s: float = 0.1) -> torch.Tensor:
    # Force numerically sensitive relational loss to fp32 and guard degenerate N.
    z = z.float()
    s = s.float()
    n = int(z.shape[0])
    if n < 2:
...
def run_train(args: argparse.Namespace) -> Dict[str, str]:
    ...
    while global_step < int(train_cfg.max_steps):
        ...
        if exhausted_any or pass_input_tokens_global >= 9447579830*args.dsize:
            passes_completed += 1
            if accelerator.is_main_process:
                print(
                ...
                if rel_proj is not None:
                    ...
                    with torch.autocast(device_type=dev.type, enabled=False):
                        ...
                        if n_rel >= 2 and torch.isfinite(patches_flat).all() and torch.isfinite(z_flat).all():
                            ...
                            if spec_target.shape[0] > M:
                                idx = torch.randperm(spec_target.shape[0], device=x.device)[:M]
                                z_flat = z_flat[idx]
                                spec_target = spec_target[idx]
                            z_rel = rel_proj(z_flat)
                            if torch.isfinite(spec_target).all() and torch.isfinite(z_rel).all():
                                ...
                                _append_jsonl(os.environ.get("EEGFM_DEBUG_LOG"), {
                                    ...
                                    "n_rel": n_rel,
                                    "patches_finite": bool(torch.isfinite(patches_flat).all().item()),
                                    "z_flat_finite": bool(torch.isfinite(z_flat).all().item()),
                                    "spec_target_finite": bool(torch.isfinite(spec_target).all().item()),
                                    "z_rel_finite": bool(torch.isfinite(z_rel).all().item()),
                                })
                                spec_rel_loss = None
                        else:
                            _append_jsonl(os.environ.get("EEGFM_DEBUG_LOG"), {
                                ...
                                "n_rel": n_rel,
                                "patches_finite": bool(torch.isfinite(patches_flat).all().item()) if patches_flat.numel() > 0 else True,
                                "z_flat_finite": bool(torch.isfinite(z_flat).all().item()) if z_flat.numel() > 0 else True,
                            })
                            spec_rel_loss = None
