...
CUDA_VISIBLE_DEVICES=0,1 python -m eegfm_runonly.ISRUC_pp \
--ckpt "/home/conelab/ft_runsD1/final_encoder" \
--isruc_root "/home/conelab/ISRUC_pp" \
--tasks isruc \
--seq_lpft \
--seq_len 20 \
--seq_stride 1 \
--seq_lp_batch_size 512 \
--seq_d_model 512 \
--seq_nhead 8 \
--seq_ff_mult 4.0 \
--seq_dropout 0.1 \
...
--num_workers 8  \
--lrs 3e-4 1e-3 3e-3 \
--ft_lrs 1e-5 3e-5 1e-4 \
--ft_train_scope full \
--ft_layer_decay 1.0 \
--ft_l2sp 0 \
--no_ft_lora \
--ft_batch_size 64 \
--wandb_name eval_ftD200_ISRUC_pp
"""
...
class NpySplitDataset(Dataset):
    def __init__(self, split_dir: str):
        ...
        # Optional grouping metadata for sequence tasks.
        self.group_ids = None
        self.group_source = None
        for extra_name in ("group.npy", "recording_id.npy", "subject_id.npy"):
            extra_path = os.path.join(split_dir, extra_name)
            if os.path.exists(extra_path):
                extra = np.load(extra_path, mmap_mode="r")
                if extra.shape[0] != self.eeg.shape[0]:
                    raise ValueError(
                        f"{extra_name} length mismatch: {extra.shape[0]} vs N={self.eeg.shape[0]}"
                    )
                self.group_ids = extra
                self.group_source = extra_name
                break
...
class EEGHead(nn.Module):
    def __init__(self, feat_dim:int, n_classes: int, layers=1):
        super().__init__()
        if layers == 3:
...
class SleepSequenceClassifier(nn.Module):
    """Pretrained sample encoder + one-layer Transformer sequence encoder."""
    ...
    def __init__(
        ...
        self.feat_dim = int(feature_model.feat_dim)
        self.seq_d_model = int(seq_d_model)

        self.input_proj = nn.Identity() if self.feat_dim == self.seq_d_model else nn.Linear(self.feat_dim, self.seq_d_model)
        self.pos_embed = nn.Parameter(torch.zeros(1, self.seq_len, self.seq_d_model))
        nn.init.normal_(self.pos_embed, mean=0.0, std=0.02)

        enc_layer = nn.TransformerEncoderLayer(
            d_model=self.seq_d_model,
            nhead=int(seq_nhead),
            dim_feedforward=int(self.seq_d_model * float(seq_ff_mult)),
            dropout=float(seq_dropout),
            activation="gelu",
            batch_first=True,
            norm_first=True,
        )
        self.seq_encoder = nn.TransformerEncoder(enc_layer, num_layers=1)
        self.head = nn.Linear(self.seq_d_model, int(n_classes))

    def forward(self, eeg: torch.Tensor, coord: torch.Tensor) -> torch.Tensor:
        if eeg.ndim != 4:
            raise ValueError(f"Expected eeg (B,S,C,T), got {tuple(eeg.shape)}")
        if coord.ndim != 4:
            raise ValueError(f"Expected coord (B,S,C,3), got {tuple(coord.shape)}")

        B, S, C, T = eeg.shape
        eeg_f = eeg.reshape(B * S, C, T)
        coord_f = coord.reshape(B * S, C, 3)

        feat = self.feature_model(eeg_f, coord_f)
        feat = self.feature_norm(feat)
...
@torch.no_grad()
def balanced_accuracy_from_cm(cm: torch.Tensor) -> float:
    """Balanced accuracy: macro-average of per-class recall over classes present in y_true."""
    cm = cm.to(torch.float32)
    support = cm.sum(dim=1)
    valid = support > 0
    if not bool(valid.any()):
        return 0.0
    recall = torch.diagonal(cm)[valid] / support[valid].clamp_min(1e-12)
...
def metrics_from_cm(cm: torch.Tensor) -> Metrics:
    ...
    acc = balanced_accuracy_from_cm(cmf)
...
def evaluate_head(
    ...
    *,
    ...
    if binary_test_metrics and n_classes == 2:
        ...
        acc = balanced_accuracy_from_cm(cm)
...
def _resolve_group_ids_from_dataset(ds: Dataset):
    if isinstance(ds, RemappedLabelDataset):
        return _resolve_group_ids_from_dataset(ds.base)
    if isinstance(ds, Subset):
        base_groups = _resolve_group_ids_from_dataset(ds.dataset)
        if base_groups is None:
            return None
        idx = np.asarray(ds.indices, dtype=np.int64)
        return np.asarray(base_groups)[idx]
    if isinstance(ds, NpySplitDataset):
        return getattr(ds, "group_ids", None)
    return None


def collect_flat_labels(ds: Dataset) -> torch.Tensor:
    ys = []
...
class SleepSequenceDataset(Dataset):
    """Build contiguous sequence windows without crossing group boundaries."""

    def __init__(self, base: Dataset, seq_len: int = 20, stride: int = 1, require_groups: bool = True):
        self.base = base
        self.seq_len = int(seq_len)
        self.stride = int(stride)
        self.require_groups = bool(require_groups)
        if self.seq_len <= 0:
            raise ValueError("seq_len must be > 0")
        if self.stride <= 0:
            raise ValueError("stride must be > 0")

        group_ids = _resolve_group_ids_from_dataset(base)
        if group_ids is None:
            if self.require_groups:
                raise ValueError(
                    "Sequence evaluation requires group.npy / recording_id.npy / subject_id.npy in each split."
                )
            group_ids = np.zeros((len(base),), dtype=np.int64)
        else:
        ...
        while start < n:
            ...
            while end < n and group_ids[end] == gid:
                end += 1

            segment = np.arange(start, end, dtype=np.int64)
            if len(segment) >= self.seq_len:
                for s in range(0, len(segment) - self.seq_len + 1, self.stride):
                    self.windows.append(segment[s:s + self.seq_len])
            start = end

        if not self.windows:
            raise ValueError(
                f"No valid sequence windows were created (seq_len={self.seq_len}, stride={self.stride})."
            )
    ...
    def __getitem__(self, idx: int):
        ...
        eegs, coords, ys = [], [], []
        for i in indices:
            eeg, coord, y = self.base[int(i)]
            eegs.append(eeg)
            coords.append(coord)
            ys.append(int(y))
        eeg = np.stack(eegs, axis=0)      # (S,C,T)
        coord = np.stack(coords, axis=0)  # (S,C,3)
        y = np.asarray(ys, dtype=np.int64)
        return eeg, coord, y


def make_collate_eeg_sequence(coord_scale: float = 10.0):
    coord_scale = float(coord_scale)
...
class SequenceFeatureEpochDataset(Dataset):
    """Epoch-level cached features with optional grouping metadata."""
    def __init__(self, X: torch.Tensor, y: torch.Tensor, group_ids: Optional[np.ndarray] = None):
        if X.ndim != 2:
            raise ValueError(f"X must be (N,D), got {tuple(X.shape)}")
        if y.ndim != 1:
            raise ValueError(f"y must be (N,), got {tuple(y.shape)}")
        if X.shape[0] != y.shape[0]:
            raise ValueError(f"N mismatch: X={tuple(X.shape)} y={tuple(y.shape)}")
        self.X = X.detach().cpu().float().contiguous()
        self.y = y.detach().cpu().long().contiguous()
        if group_ids is None:
            self.group_ids = np.zeros((self.X.shape[0],), dtype=np.int64)
        else:
            group_ids = np.asarray(group_ids)
            if group_ids.shape[0] != self.X.shape[0]:
                raise ValueError(f"group_ids mismatch: {group_ids.shape[0]} vs N={self.X.shape[0]}")
...
class SleepSequenceFeatureDataset(Dataset):
    """Sequence windows over cached epoch features, without crossing group boundaries."""
    def __init__(self, base: SequenceFeatureEpochDataset, seq_len: int = 20, stride: int = 1):
        self.base = base
        self.seq_len = int(seq_len)
        self.stride = int(stride)
        if self.seq_len <= 0:
        ...
        group_ids = np.asarray(base.group_ids)
        self.windows = []
        n = len(base)
        start = 0
        while start < n:
            ...
            seg = np.arange(start, end, dtype=np.int64)
            if len(seg) >= self.seq_len:
                for s in range(0, len(seg) - self.seq_len + 1, self.stride):
                    self.windows.append(seg[s:s + self.seq_len])
            start = end

        if not self.windows:
            raise ValueError(f"No valid cached feature sequence windows were created (seq_len={self.seq_len}, stride={self.stride}).")
    ...
    def __getitem__(self, idx: int):
        indices = self.windows[idx]
        X_seq = self.base.X[indices]                     # (S,D)
        y_seq = self.base.y[indices]                     # (S,)
        return X_seq, y_seq


def make_collate_sequence_features():
    def _collate(batch):
        xs, ys = zip(*batch)
        x = torch.stack(xs, dim=0).float()              # (B,S,D)
        y = torch.stack(ys, dim=0).long()               # (B,S)
        return x, y
    return _collate


class SleepSequenceHead(nn.Module):
    """
    LP-time head only:
      cached epoch features -> feature_norm -> input_proj -> 1-layer transformer -> classifier

    The parameter names intentionally match the non-encoder part of SleepSequenceClassifier,
    so the resulting state_dict can be loaded into SleepSequenceClassifier for FT.
    """
    def __init__(
        self,
        feat_dim: int,
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        B, S, D = x.shape
        x = self.feature_norm(x.reshape(B * S, D)).view(B, S, D)
        x = self.input_proj(x)
        x = x + self.pos_embed[:, :S, :]
        x = self.seq_encoder(x)
        return self.head(x)
...
def evaluate_sequence_head(
    ...
    for xb, yb in loader:
        ...
        logits = model(xb)
        pred = logits.argmax(dim=-1)
        cm += _confusion_matrix(yb.reshape(-1), pred.reshape(-1), n_classes=n_classes, device=device)
    return metrics_from_cm(cm)
...
def train_linear_probe(
    ...
    for ep in range(1, epochs + 1):
        head.train()
        train_cm = torch.zeros((n_classes, n_classes), dtype=torch.long, device=device)
        for xb, yb in train_loader:
            ...
            logits = head(xb)
            pred = logits.argmax(dim=-1)
            train_cm += _confusion_matrix(yb, pred, n_classes=n_classes, device=device)
            loss = loss_fn(logits, yb)
            loss.backward()
            opt.step()

        train_acc = balanced_accuracy_from_cm(train_cm)
...
def _build_param_groups_for_named_params(named_params, *, lr: float, weight_decay: float, group_name: str):
    decay_params = []
    no_decay_params = []
    ...
    groups = []
    if decay_params:
        groups.append({"params": decay_params, "lr": float(lr), "weight_decay": float(weight_decay), "group_name": str(group_name)})
    if no_decay_params:
        groups.append({"params": no_decay_params, "lr": float(lr), "weight_decay": 0.0, "group_name": f"{group_name}/no_decay"})
    return groups


def _sequence_nonencoder_named_params(model: SleepSequenceClassifier):
    for name, param in model.named_parameters():
        if name.startswith("feature_model.encoder."):
            continue
        yield name, param


def _sequence_head_state_dict(model: nn.Module) -> Dict[str, torch.Tensor]:
    base = unwrap_module(model)
    state = {}
    for k, v in base.state_dict().items():
        if k.startswith("feature_model.encoder."):
            continue
        state[k] = v.detach().cpu().clone()
    return state


def _load_sequence_head_state_dict(model: SleepSequenceClassifier, state_dict: Dict[str, torch.Tensor]) -> None:
    missing, unexpected = model.load_state_dict(copy.deepcopy(state_dict), strict=False)
    unexpected = [k for k in unexpected if not k.startswith("feature_model.encoder.")]
    if unexpected:
        raise RuntimeError(f"Unexpected non-encoder keys when loading sequence head state: {unexpected}")
...
def _make_eeg_sequence_loader(
    ...
    *,
    ...
    loader_kwargs = dict(
        ...
        pin_memory=pin_memory,
        drop_last=False,
        collate_fn=make_collate_eeg_sequence(coord_scale),
        persistent_workers=(num_workers > 0),
    )
    if num_workers > 0:
        loader_kwargs["prefetch_factor"] = 2
...
def train_sequence_lp(
    train_ds: Dataset,
    val_ds: Dataset,
    test_ds: Dataset,
    y_train_epoch: torch.Tensor,
    n_classes: int,
    norm_stats: FeatureNormStats,
    device: torch.device,
    device_ids: List[int],
    ...
    """
    LP for sequence setting:
      1) encoder features are pre-extracted for every epoch
      2) one-layer Transformer sequence head is trained on cached feature sequences only
    """
    set_seed(seed)

    # infer feature dimension from cached feature sequences
    sample_x, _ = train_ds[0]
    feat_dim = int(sample_x.shape[-1])

    model = SleepSequenceHead(
        feat_dim=feat_dim,
        n_classes=n_classes,
        ...
        seq_nhead=seq_nhead,
        seq_ff_mult=seq_ff_mult,
        seq_dropout=seq_dropout,
    ).to(device)

    param_groups = _build_param_groups_for_named_params(
    ...
    if class_weight == "balanced":
        counts = torch.bincount(y_train_epoch.cpu(), minlength=n_classes).float()
        w = (counts.sum() / counts.clamp_min(1.0))
        w = (w / w.mean()).to(device)
        loss_fn = nn.CrossEntropyLoss(weight=w)
    else:
    ...
    train_loader = DataLoader(
        ...
        pin_memory=True,
        drop_last=False,
        collate_fn=make_collate_sequence_features(),
    )
    ...
    for ep in range(1, epochs + 1):
        ...
        pbar = None
        if tqdm is not None:
            pbar = tqdm(train_loader, total=len(train_loader), desc=f"[SEQ-LP {lr:.1e}] ep {ep}/{epochs}", leave=False, dynamic_ncols=True)
            train_iter = pbar
        ...
        for step, (xb, yb) in enumerate(train_iter, start=1):
            ...
            pred = logits.argmax(dim=-1)
            train_cm += _confusion_matrix(yb.reshape(-1), pred.reshape(-1), n_classes=n_classes, device=device)
            loss_sum += float(loss.detach().item())
            loss.backward()
            opt.step()
            if pbar is not None:
                pbar.set_postfix(loss=f"{loss_sum/max(step,1):.4f}", acc=f"{balanced_accuracy_from_cm(train_cm):.4f}")

        train_acc_last = float(balanced_accuracy_from_cm(train_cm))
        last_epoch = int(ep)
        val_metrics = evaluate_sequence_head(model, val_loader, n_classes=n_classes, device=device)
        test_metrics = evaluate_sequence_head(model, test_loader, n_classes=n_classes, device=device)
        ...
        if val_metrics.acc > best_val + 1e-6:
            ...
            best_test_f1w = float(test_metrics.f1w)
            best_test_kappa = float(test_metrics.kappa)
            best_state = {k: v.detach().cpu().clone() for k, v in unwrap_module(model).state_dict().items()}
            train_acc_at_best = float(train_acc_last)
            bad = 0
        else:
            bad += 1
        ...
        print(f"[SEQ-LP][ep {ep}/{epochs}] train_acc={train_acc_last:.4f} val_acc={val_metrics.acc:.4f} best_val={best_val:.4f} bad={bad}/{patience}")
...
def _set_seq_lpft_train_mode(model: SleepSequenceClassifier, trainable_encoder_modules: Sequence[nn.Module]) -> None:
    model.input_proj.train()
    model.seq_encoder.train()
    model.head.train()
    encoder = model.feature_model.encoder
    encoder.eval()
    for module in trainable_encoder_modules:
...
def train_sequence_lpft_finetune(
    ckpt_dir: str,
    train_ds: Dataset,
    val_ds: Dataset,
    test_ds: Dataset,
    ...
    _load_sequence_head_state_dict(model, lp_head_state)
    model.to(device)
    ...
    head_param_groups = _build_param_groups_for_named_params(
        list(_sequence_nonencoder_named_params(base_model)),
        lr=float(head_lr),
        weight_decay=float(weight_decay),
        group_name="seq_head",
    )
    ...
    train_loader = _make_eeg_sequence_loader(train_ds, batch_size=batch_size, shuffle=True, seed=seed, num_workers=num_workers, pin_memory=pin_memory, coord_scale=coord_scale)
    val_loader = _make_eeg_sequence_loader(val_ds, batch_size=batch_size, shuffle=False, seed=seed, num_workers=num_workers, pin_memory=pin_memory, coord_scale=coord_scale)
    test_loader = _make_eeg_sequence_loader(test_ds, batch_size=batch_size, shuffle=False, seed=seed, num_workers=num_workers, pin_memory=pin_memory, coord_scale=coord_scale)

    total_steps = int(max(1, epochs * max(len(train_loader), 1)))
    scheduler = make_warmup_cosine_scheduler(optimizer, total_steps=total_steps, warmup_ratio=warmup_ratio, min_lr_ratio=min_lr_ratio)
    ...
    for ep in range(1, epochs + 1):
        model.train()
        _set_seq_lpft_train_mode(base_model, trainable_encoder_modules)
        train_cm = torch.zeros((n_classes, n_classes), dtype=torch.long, device=device)
        loss_sum = 0.0
        train_iter = train_loader
        pbar = None
        if tqdm is not None:
            pbar = tqdm(train_loader, total=len(train_loader), desc=f"[SEQ-FT {backbone_lr:.1e}] ep {ep}/{epochs}", leave=False, dynamic_ncols=True)
            train_iter = pbar

        for step, (eeg, coord, yb) in enumerate(train_iter, start=1):
            eeg = eeg.to(device, non_blocking=True)
            coord = coord.to(device, non_blocking=True)
            yb = yb.to(device, non_blocking=True)
            ...
            if pbar is not None:
                ...
                pbar.set_postfix(loss=f"{loss_sum/step:.4f}", acc=f"{balanced_accuracy_from_cm(train_cm):.4f}", lr=f"{cur_lr:.2e}")

        train_acc = balanced_accuracy_from_cm(train_cm)
        train_acc_last = float(train_acc)
        last_epoch = ep
        val_metrics = evaluate_sequence_classifier(model, val_loader, n_classes=n_classes, device=device)
        if val_metrics.acc > best_val + 1e-6:
            test_metrics = evaluate_sequence_classifier(model, test_loader, n_classes=n_classes, device=device)
            best_val = float(val_metrics.acc)
            best_epoch = ep
            best_test_acc = float(test_metrics.acc)
            best_val_f1w = float(val_metrics.f1w)
        ...
        print(f"[SEQ-FT][ep {ep}/{epochs}] train_acc={train_acc_last:.4f} val_acc={val_metrics.acc:.4f} best_val={best_val:.4f} bad={bad}/{patience}")
        if bad >= patience:
            print(f"[SEQ-FT] early stopping at epoch {ep}")
            break
...
def train_lpft_finetune(
    ...
    lp_state = copy.deepcopy(lp_head_state)

    # LP stage used bare nn.Linear -> keys are "weight", "bias"
    # FT stage uses EEGHead wrapping inner module self.head -> keys become "head.weight", "head.bias"
    if "weight" in lp_state and "bias" in lp_state:
        lp_state = {
            "head.weight": lp_state["weight"],
            "head.bias": lp_state["bias"],
        }

    model.head.load_state_dict(lp_state, strict=True)
    model.to(device)
    ...
    for ep in range(1, epochs + 1):
        ...
        if tqdm is not None:
            pbar = tqdm(
                ...
                desc=f"[FT {backbone_lr:.1e}] ep {ep}/{epochs}",
                leave=False,
                dynamic_ncols=True,
            )
            train_iter = pbar
        ...
        for step, (eeg, coord, yb) in enumerate(train_iter, start=1):
            ...
            if pbar is not None:
                cur_lr = optimizer.param_groups[0]["lr"]
                pbar.set_postfix(
                    loss=f"{loss_sum / step:.4f}",
                    acc=f"{balanced_accuracy_from_cm(train_cm):.4f}",
                    lr=f"{cur_lr:.2e}",
                )
        ...
        if bad >= patience:
            print(f"[FT] early stopping at epoch {ep}")
            break
...
def build_parser(
    ...
    *,
    ...
    ap.add_argument("--patience", type=int, default=5, help="LP early-stopping patience.")
    ...
    # Sequence LPFT (CBraMod-style downstream head)
    add_bool_arg(ap, "seq_lpft", default=False, help_text="Run LP->FT with a one-layer Transformer sequence encoder on top of the pretrained sample encoder.")
    ap.add_argument("--seq_len", type=int, default=20)
    ap.add_argument("--seq_stride", type=int, default=1)
    ap.add_argument("--seq_lp_batch_size", type=int, default=16)
    ap.add_argument("--seq_d_model", type=int, default=512)
    ap.add_argument("--seq_nhead", type=int, default=8)
    ap.add_argument("--seq_ff_mult", type=float, default=4.0)
    ap.add_argument("--seq_dropout", type=float, default=0.1)
    add_bool_arg(ap, "seq_require_groups", default=True, help_text="Require group.npy / recording_id.npy / subject_id.npy for sequence window construction.")
...
def run_eval(args: argparse.Namespace) -> List[Dict[str, object]]:
    ...
    if args.lpft and args.seq_lpft:
        raise ValueError("Use either --lpft (sample-wise) or --seq_lpft (sequence LPFT), not both together.")
    ...
    print(
        ...
        f"amp={args.amp} num_workers={args.num_workers} pin_memory={args.pin_memory} mode={'SEQ-LPFT' if args.seq_lpft else ('LPFT' if args.lpft else 'LP')}"
    )

    task_specs = resolve_task_specs(args)
    print(f"[eval] tasks={', '.join([f'{task}:{root}' for task, root in task_specs])}")
    if args.lpft or args.seq_lpft:
        ...
        print(
            f"[eval] {'SEQ-' if args.seq_lpft else ''}LPFT config: ft_epochs={ft_epochs} ft_patience={ft_patience} ft_batch_size={args.ft_batch_size} "
        ...
        if args.seq_lpft:
            print(
                f"[eval] SEQ config: seq_len={args.seq_len} seq_stride={args.seq_stride} seq_lp_batch_size={args.seq_lp_batch_size} "
                f"seq_ft_batch_size={args.ft_batch_size} seq_d_model={args.seq_d_model} seq_nhead={args.seq_nhead} "
                f"seq_ff_mult={args.seq_ff_mult} seq_dropout={args.seq_dropout} require_groups={args.seq_require_groups}"
            )
    ...
    if use_wandb:
        ...
        wandb.init(
            ...
            config={
                ...
                "seed": seed,
                "mode": "seq_lpft" if args.seq_lpft else ("lpft" if args.lpft else "lp"),
    ...
    for ckpt_idx, ckpt_dir in enumerate(ckpt_dirs):
        ...
        for task, root in task_specs:
            ...
            if args.seq_lpft:
                print(
                    f"[task={task}] building sequence datasets: seq_len={args.seq_len} stride={args.seq_stride} "
                    f"train_epochs={len(train_ds)} val_epochs={len(val_ds)} test_epochs={len(test_ds)}"
                )
                seq_train_ds = SleepSequenceDataset(train_ds, seq_len=int(args.seq_len), stride=int(args.seq_stride), require_groups=bool(args.seq_require_groups))
                seq_val_ds = SleepSequenceDataset(val_ds, seq_len=int(args.seq_len), stride=int(args.seq_stride), require_groups=bool(args.seq_require_groups))
                seq_test_ds = SleepSequenceDataset(test_ds, seq_len=int(args.seq_len), stride=int(args.seq_stride), require_groups=bool(args.seq_require_groups))
                ytr_epoch = collect_flat_labels(train_ds)

                # Sequence LP should cache epoch features first, then train only the one-layer
                # Transformer sequence head on top of those cached features.
                print(f"[task={task}] extracting cached epoch features for sequence LP")
                Xtr_seq_raw, ytr_feat = extract_features(
                    feature_model=feature_model,
                    ds=train_ds,
                    device=device,
                    feat_batch_size=args.feat_batch_size,
                ...
                Xva_seq_raw, yva_feat = extract_features(
                    feature_model=feature_model,
                    ds=val_ds,
                    device=device,
                    feat_batch_size=args.feat_batch_size,
                ...
                Xte_seq_raw, yte_feat = extract_features(
                    feature_model=feature_model,
                    ds=test_ds,
                    device=device,
                    feat_batch_size=args.feat_batch_size,
                ...
                if str(args.feat_norm).lower().strip() == "none":
                    norm_stats = FeatureNormStats(mode="none")
                else:
                    norm_stats = compute_feature_norm_stats(Xtr_seq_raw, mode=args.feat_norm)

                Xtr_seq, Xva_seq, Xte_seq = normalize_features(Xtr_seq_raw, Xva_seq_raw, Xte_seq_raw, norm_stats)

                tr_groups = _resolve_group_ids_from_dataset(train_ds)
                va_groups = _resolve_group_ids_from_dataset(val_ds)
                te_groups = _resolve_group_ids_from_dataset(test_ds)

                feat_train_epoch_ds = SequenceFeatureEpochDataset(Xtr_seq, ytr_feat, tr_groups)
                feat_val_epoch_ds = SequenceFeatureEpochDataset(Xva_seq, yva_feat, va_groups)
                feat_test_epoch_ds = SequenceFeatureEpochDataset(Xte_seq, yte_feat, te_groups)

                seq_train_feat_ds = SleepSequenceFeatureDataset(feat_train_epoch_ds, seq_len=int(args.seq_len), stride=int(args.seq_stride))
                seq_val_feat_ds = SleepSequenceFeatureDataset(feat_val_epoch_ds, seq_len=int(args.seq_len), stride=int(args.seq_stride))
                seq_test_feat_ds = SleepSequenceFeatureDataset(feat_test_epoch_ds, seq_len=int(args.seq_len), stride=int(args.seq_stride))

                print(f"[task={task}] sequence windows: train={len(seq_train_ds)} val={len(seq_val_ds)} test={len(seq_test_ds)}")
                print(f"[task={task}] cached-feature sequence windows: train={len(seq_train_feat_ds)} val={len(seq_val_feat_ds)} test={len(seq_test_feat_ds)}")
                ...
                if args.tune_lr_on == "first_ckpt" and task in chosen_lp_lr_per_task:
                    ...
                    print(f"[task={task}] [SEQ-LP] using pre-chosen lr={use_lp_lrs[0]:.2e}")
                else:
                ...
                for lp_lr in use_lp_lrs:
                    r = train_sequence_lp(
                        train_ds=seq_train_feat_ds,
                        val_ds=seq_val_feat_ds,
                        test_ds=seq_test_feat_ds,
                        y_train_epoch=ytr_epoch,
                        n_classes=n_classes,
                        norm_stats=norm_stats,
                        device=device,
                        ...
                        batch_size=int(args.seq_lp_batch_size),
                        weight_decay=0.0,
                        class_weight=None if args.class_weight == "none" else "balanced",
                        seq_len=int(args.seq_len),
                        seq_d_model=int(args.seq_d_model),
                        seq_nhead=int(args.seq_nhead),
                        seq_ff_mult=float(args.seq_ff_mult),
                        seq_dropout=float(args.seq_dropout),
                        seed=seed,
                    )
                    print(
                        f"[task={task}] [SEQ-LP] lr={lp_lr:.2e} best_val={r.best_val_acc:.4f} "
                ...
                if args.tune_lr_on == "first_ckpt" and ckpt_idx == 0:
                    ...
                    print(f"[task={task}] [SEQ-LP] chosen lr for reuse across later checkpoints = {best_lp.lr:.2e}")

                if best_lp.state_dict is None:
                    raise RuntimeError("Sequence LP stage did not return state_dict required for sequence LPFT.")

                if args.tune_lr_on == "first_ckpt" and task in chosen_ft_lr_per_task:
                    use_ft_lrs = [chosen_ft_lr_per_task[task]]
                    print(f"[task={task}] [SEQ-FT] using pre-chosen backbone_lr={use_ft_lrs[0]:.2e}")
                else:
                    use_ft_lrs = ft_lr_grid

                best_ft = None
                for ft_lr in use_ft_lrs:
                    ft_result = train_sequence_lpft_finetune(
                        ckpt_dir=ckpt_dir,
                    ...
                    print(
                        f"[task={task}] [SEQ-FT] mode={'lora' if ft_result.use_lora else 'direct'} scope={ft_result.train_scope} "
                        f"unfreeze_last_k={ft_result.unfreeze_last_k} blocks={list(ft_result.trainable_block_indices)} "
                        f"trainable={ft_result.encoder_trainable_params/1e6:.2f}M/{ft_result.encoder_total_params/1e6:.2f}M backbone_lr={ft_lr:.2e} "
                        f"head_lr={ft_result.head_lr:.2e} layer_decay={ft_result.layer_decay:.3f} l2sp={ft_result.l2sp_weight:.2e} "
                        ...
                        f"best_ep={ft_result.best_epoch} train_last={ft_result.train_acc_last:.4f} train@best={ft_result.train_acc_at_best:.4f} stop_ep={ft_result.last_epoch}"
                    )
                ...
                if args.tune_lr_on == "first_ckpt" and ckpt_idx == 0:
                    ...
                    print(f"[task={task}] [SEQ-FT] chosen backbone_lr for reuse across later checkpoints = {best_ft.backbone_lr:.2e}")
                ...
                shared_row = dict(
                    ...
                    ft_l2sp=float(best_ft.l2sp_weight),
                    ft_use_lora=bool(best_ft.use_lora),
                    ft_lora_rank=int(best_ft.lora_rank),
                    ft_lora_alpha=float(best_ft.lora_alpha),
                    ft_lora_dropout=float(best_ft.lora_dropout),
                    ft_lora_include_spatial_qk=bool(best_ft.lora_include_spatial_qk),
                    ft_lora_wrapped_linears=int(best_ft.lora_wrapped_linears),
                ...
                csv_rows.append(dict(shared_row, train_mode="seq_lp", lr=float(best_lp.lr), best_epoch=int(best_lp.best_epoch), stop_epoch=int(best_lp.last_epoch), train_acc_last=float(best_lp.train_acc_last), train_acc_at_best=float(best_lp.train_acc_at_best), val_acc=float(best_lp.best_val_acc), val_f1w=float(best_lp.best_val_f1w), test_acc=float(best_lp.test_acc_at_best), test_f1w=float(best_lp.test_f1w_at_best), test_kappa=float(best_lp.test_kappa_at_best)))
                csv_rows.append(dict(shared_row, train_mode="seq_lpft", lr=float(best_ft.backbone_lr), best_epoch=int(best_ft.best_epoch), stop_epoch=int(best_ft.last_epoch), train_acc_last=float(best_ft.train_acc_last), train_acc_at_best=float(best_ft.train_acc_at_best), val_acc=float(best_ft.best_val_acc), val_f1w=float(best_ft.best_val_f1w), test_acc=float(best_ft.test_acc_at_best), test_f1w=float(best_ft.test_f1w_at_best), test_kappa=float(best_ft.test_kappa_at_best)))
                print(f"[task={task}] BEST[SEQ-LP]: val_acc={best_lp.best_val_acc:.4f} test_acc={best_lp.test_acc_at_best:.4f} test_f1w={best_lp.test_f1w_at_best:.4f} test_kappa={best_lp.test_kappa_at_best:.4f} lr={best_lp.lr:.2e}")
                print(f"[task={task}] BEST[SEQ-LPFT]: val_acc={best_ft.best_val_acc:.4f} test_acc={best_ft.test_acc_at_best:.4f} test_f1w={best_ft.test_f1w_at_best:.4f} test_kappa={best_ft.test_kappa_at_best:.4f} backbone_lr={best_ft.backbone_lr:.2e} head_lr={best_ft.head_lr:.2e}")
                continue
